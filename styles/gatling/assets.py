# -*- coding: utf-8 -*-
"""风格 "gatling" 的 Blender 资产：房屋、石头、树、灌木、桥。

    blender --background --factory-startup --python styles/gatling/assets.py \
        -- --out-dir assets/gatling/environment [--only trees,rocks]

通用建模辅助在 tools/blenderlib.py，所有风格共用；
这个文件只放**这个风格长什么样**：调色板 + 各类资产的建模函数。
"""
import argparse
import math
import os
...
from mathutils import Quaternion, Vector    # noqa: E402,F401

import blenderlib as bl                     # noqa: E402
from blenderlib import (box, cyl, wedge, mat, clear, join_as, finalize, tris,
                        srgb_to_linear, export, uv_planar, bevel_edges,
                        set_vcol, bake_materials_to_vcol)   # noqa: E402,F401
...
# 调色板注入 blenderlib：mat() 从那里查颜色。
# 必须在任何 mat() 调用之前执行 —— 拆分时这一行一度排在 main() 之后，
# 结果 main() 先跑，mat() 在空调色板上 KeyError。
...
def building(name, w, d, wall_h, loc, roof_h=1.9, eaves=0.45, storeys=1,
    ...
    """
    import random
    # 用 crc32 而不是 hash()：Python 3 的 str.__hash__ 每个进程都加了随机盐，
    # 所以 hash(name) 会让**每次导出的资产都不一样** —— 屋脊、檐口、地基石
    # 的碎件全都在变。资产生成必须是确定的，否则没法做"改前/改后"的像素对比。
    rng = random.Random(zlib.crc32(name.encode("utf-8")) & 0xffff)
    parts = []
    base_h = 0.28
