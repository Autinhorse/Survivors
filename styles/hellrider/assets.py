...
# 配石的偏移要让它和主石**只挨着、不重叠**：
#     距离 >= (主石半径 + 配石半径) x 1.06(flare) x 1.03(留缝)
# 原来的偏移是重叠 0.14-0.35 m 的。石头是上收的，所以重叠的配石会把
# 自己的顶角从主石的斜面里**捅出来** —— 读出来就是"大石头上放了块小石头，
# 还露出个角"。挨着是对的（参考图里石头就是成组挤在一起的），
# 插进去不对。
ROCK_GROUPS = {
    "big":   [(1.85, 0.70, 0.75, 6, 0.00, 0.00),
              (0.85, 0.34, 0.34, 5, 2.72, -1.13),
              (0.50, 0.20, 0.20, 5, -2.26, 1.22)],
    "twin":  [(1.45, 0.62, 0.62, 6, 0.00, 0.00),
              (1.05, 0.48, 0.48, 5, 2.54, 0.99)],
    "spire": [(1.00, 0.85, 0.85, 5, 0.00, 0.00),
              (0.60, 0.26, 0.26, 5, 1.52, -0.86)],
    "low":   [(1.95, 0.45, 0.45, 7, 0.00, 0.00),
              (0.75, 0.30, 0.30, 5, 2.70, 1.19)],
    "block": [(1.20, 0.72, 0.72, 5, 0.00, 0.00)],
    "slab":  [(2.10, 0.36, 0.36, 7, 0.00, 0.00),
              (0.48, 0.18, 0.18, 5, 2.69, -0.82)],
}
